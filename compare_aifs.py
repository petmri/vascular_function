...
for key, value in aif_values.items():
    ...
    if 'manual_ktrans' in value and 'auto_ktrans' in value:
        ...
        valid_manual_ktrans = valid_manual_ktrans[valid_manual_ktrans > ktrans_zero_threshold]
        if valid_manual_ktrans.size == 0:
            manual_ktrans_mean = 0
            print(f"Manual Ktrans for {key} is all zeros")
        else:
            manual_ktrans_mean = np.median(valid_manual_ktrans)*1000
        ...
        valid_auto_ktrans = valid_auto_ktrans[valid_auto_ktrans > ktrans_zero_threshold]
        if valid_auto_ktrans.size == 0:
            auto_ktrans_mean = 0
            print(f"Auto Ktrans for {key} is all zeros")
        else:
            auto_ktrans_mean = np.median(valid_auto_ktrans)*1000
...
if any(np.array(manual_ktrans_list)>max_axis) or any(np.array(auto_ktrans_list)>max_axis):
    ...
    print("Manual Ktrans values above max_axis:", np.array(manual_ktrans_list)[np.array(manual_ktrans_list) > max_axis])
...
# print warning if any values are above the max_axis
if any(np.array(manual_all) > max_axis) or any(np.array(auto_all) > max_axis):
    print(f"Warning: GM, WM, Cerebellum Ktrans value not displayed on plot value above {max_axis}")
    if np.any(np.array(manual_all) > max_axis): print("Manual Ktrans values above max_axis:", np.array(manual_all)[(np.array(manual_all) > max_axis)])
    if np.any(np.array(auto_all) > max_axis): print("Auto Ktrans values above max_axis:", np.array(auto_all)[(np.array(auto_all) > max_axis)])
...
with open(os.path.join(output_dir, 'results_statistics.txt'), 'a') as f:
    f.write(f"\nPaired t-test of AIFitness values: t-statistic = {t_statistic[0]:.3f}, p-value = {t_statistic[1]:.3f}\n")
...
with open(os.path.join(output_dir, 'results_statistics.txt'), 'a') as f:
    f.write(f"\nIntraclass Correlation Coefficient Results:\n")
    f.write(f"ICC2: {icc.loc[icc['Type'] == 'ICC2', 'ICC'].values[0]:.3f}\n")
    f.write(f"95% CI: [{icc.loc[icc['Type'] == 'ICC2', 'CI95%'].values[0][0]:.3f}, {icc.loc[icc['Type'] == 'ICC2', 'CI95%'].values[0][1]:.3f}]\n")
...
print(f"Manual Ktrans: Median = {np.median(manual_ktrans_array):.2f}, Standard Dev = {np.std(manual_ktrans_array):.2f}")
print(f"Auto Ktrans: Median = {np.median(auto_ktrans_array):.2f}, Standard Dev = {np.std(auto_ktrans_array):.2f}")
...
print(f"Manual GM Ktrans: Median = {np.median(manual_ktrans_GM_array):.2f}, Standard Dev = {np.std(manual_ktrans_GM_array):.2f}")
print(f"Auto GM Ktrans: Median = {np.median(auto_ktrans_GM_array):.2f}, Standard Dev = {np.std(auto_ktrans_GM_array):.2f}")
print(f"Manual WM Ktrans: Median = {np.median(manual_ktrans_WM_array):.2f}, Standard Dev = {np.std(manual_ktrans_WM_array):.2f}")
print(f"Auto WM Ktrans: Median = {np.median(auto_ktrans_WM_array):.2f}, Standard Dev = {np.std(auto_ktrans_WM_array):.2f}")
print(f"Manual Cerebellum Ktrans: Median = {np.median(manual_ktrans_cerb_array):.2f}, Standard Dev = {np.std(manual_ktrans_cerb_array):.2f}")
print(f"Auto Cerebellum Ktrans: Median = {np.median(auto_ktrans_cerb_array):.2f}, Standard Dev = {np.std(auto_ktrans_cerb_array):.2f}")
print(f"Manual Muscle Ktrans: Median = {np.median(manual_ktrans_muscle_array):.2f}, Standard Dev = {np.std(manual_ktrans_muscle_array):.2f}")
print(f"Auto Muscle Ktrans: Median = {np.median(auto_ktrans_muscle_array):.2f}, Standard Dev = {np.std(auto_ktrans_muscle_array):.2f}")
with open(os.path.join(output_dir, 'results_statistics.txt'), 'a') as f:
    f.write(f"\nMedian and Standard Deviation of Ktrans values\n")
    f.write(f"Manual Ktrans: Median = {np.median(manual_ktrans_array):.2f}, Standard Dev = {np.std(manual_ktrans_array):.2f}\n")
    f.write(f"Auto Ktrans: Median = {np.median(auto_ktrans_array):.2f}, Standard Dev = {np.std(auto_ktrans_array):.2f}\n")
    f.write(f"Manual AIFitness: Mean = {np.mean(manual_aifitness_array):.2f}, Standard Dev = {np.std(manual_aifitness_array):.2f}\n")
    f.write(f"Auto AIFitness: Mean = {np.mean(auto_aifitness_array):.2f}, Standard Dev = {np.std(auto_aifitness_array):.2f}\n")
    f.write(f"Manual AIFitness: 5th Percentile = {np.percentile(manual_aifitness_array, 5):.2f}\n")
    f.write(f"Auto AIFitness: 5th Percentile = {np.percentile(auto_aifitness_array, 5):.2f}\n")
    f.write(f"Manual GM Ktrans: Median = {np.median(manual_ktrans_GM_array):.2f}, Standard Dev = {np.std(manual_ktrans_GM_array):.2f}\n")
    f.write(f"Auto GM Ktrans: Median = {np.median(auto_ktrans_GM_array):.2f}, Standard Dev = {np.std(auto_ktrans_GM_array):.2f}\n")
    f.write(f"Manual WM Ktrans: Median = {np.median(manual_ktrans_WM_array):.2f}, Standard Dev = {np.std(manual_ktrans_WM_array):.2f}\n")
    f.write(f"Auto WM Ktrans: Median = {np.median(auto_ktrans_WM_array):.2f}, Standard Dev = {np.std(auto_ktrans_WM_array):.2f}\n")
    f.write(f"Manual Cerebellum Ktrans: Median = {np.median(manual_ktrans_cerb_array):.2f}, Standard Dev = {np.std(manual_ktrans_cerb_array):.2f}\n")
    f.write(f"Auto Cerebellum Ktrans: Median = {np.median(auto_ktrans_cerb_array):.2f}, Standard Dev = {np.std(auto_ktrans_cerb_array):.2f}\n")
    f.write(f"Manual Muscle Ktrans: Median = {np.median(manual_ktrans_muscle_array):.2f}, Standard Dev = {np.std(manual_ktrans_muscle_array):.2f}\n")
    f.write(f"Auto Muscle Ktrans: Median = {np.median(auto_ktrans_muscle_array):.2f}, Standard Dev = {np.std(auto_ktrans_muscle_array):.2f}\n")
